...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @dotpp/ai <command> [provider]
...
Examples:
  npx @dotpp/ai login              # interactive provider selection
  npx @dotpp/ai login anthropic    # login to specific provider
  npx @dotpp/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @dotpp/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @dotpp/ai --help' for usage`);
	process.exit(1);
}
